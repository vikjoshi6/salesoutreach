...
async function createDraft(config: AppConfig, lead: Lead, audit: Audit, mockup: Mockup): Promise<OutreachDraft> {
  ...
  const body = [
    ...
    "",
    `I was looking at local ${segmentLabel(lead.segment)} companies around ${config.TARGET_METRO} and noticed a few places where the first website visit could turn into a quote request faster.`,
    `The biggest opportunity I saw: ${audit.observations[0]}`,
    "",
    `I put together a small concept preview here: ${mockup.url}`,
    "",
    `I run ${config.AGENCY_NAME}, where we help service businesses with websites, social content, and practical AI intake workflows. If this is worth a quick look, reply here and I can send over a tighter version for your business.`,
